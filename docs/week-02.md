...
```
A조에서는 스키마·규약을  제공받지 못했기 때문에 컬럼명(content vs body), 파라미터명(keyword vs q), 응답 형식(배열 그대로 vs {ok, data})을 전부 임의로 지어내게 되었고 무엇보다 사용자 구분(user_id) 개념 자체가 없어서 검색을 하면 모든 사용자의 메모가 뒤섞여 나오는 걸 볼 수 있었습니다. 반면 B조는 실제 schema.sql·service.js·routes.js·규약을 그대로 따랐기 때문에 기존 코드에 바로 병합할 수 있고, 본인 메모만 반환된다는 것을 확인 할 수 있었습니다.
```

**4-2. A조의 실패는 모델 탓입니까, 우리가 주지 않은 탓입니까? 근거를 들어 적으세요.**

```
스키마와 규약을 제공해주지 않은 탓이 더 크다고 생각합니다. 클로드라는 동일한 모델에서 B 부분의 대화에서는 스키마·규약·기존 코드를 받고 나서 컬럼명, 응답 형식, 에러 코드 규칙, user_id 스코핑까지 전부 정확히 지켰고, 실제로 9개 테스트를 통과한 것을 확인했기 때문입니다. 즉 모델이 지시를 못 따라서가 아니라, DB 구조나 응답 규칙, 권한 모델처럼 "그 프로젝트만의 약속"은 프롬프트에 적혀 있지 않으면 모델이 알 방법이 없어서 어쩔 수 없이 추측하게 된다고 생각하여 A의 실패는 정보 부족의 결과지  모델 능력의 한계는 아니라고 생각합니다.
```

**4-3. B조가 준 자료 중 결과를 가장 크게 바꾼 것 하나를 꼽는다면 무엇입니까? 왜 그렇게 생각합니까?**

```
B조가 준 자료 중 결과를 가장 크게 바꾼 것은 규약 문서의 권한 조항, 즉 "모든 조회와 수정은 본인 소유 데이터로 한정하며 모든 쿼리에 user_id 조건을 포함한다"는 규칙이었다고 생각합니다. schema.sql와 기존 코드는 컬럼명이나 구현 패턴 등 형식적인 부분을 알려주는 자료였던 반면 앞서 말한 조항은 본인 메모만 검색되어야 한다는 안전성 요건을 명확한 문장으로 직접 지시한 유일한 자료였기 때문입니다. 
```
